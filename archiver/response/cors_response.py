...
from archiver.openapi_server.models import Status400BadRequestErrors, Status400BadRequest, Status401UnauthorizedErrors, \
    Status401Unauthorized, Status403ForbiddenErrors, Status403Forbidden, Status404NotFoundErrors, Status404NotFound, \
    Status500InternalServerErrorErrors, Status500InternalServerError, Status200OkNoContentData, Status200OkNoContent, \
    Status409ConflictErrors, Status409Conflict
...
def cors_200(response_body = None) -> Response:
    """
    Return 200 - OK
    """
...
def cors_409(details: str = None) -> Response:
    """
    Return 409 - Conflict
    """
    errors = Status409ConflictErrors()
    errors.details = details
    error_object = Status409Conflict([errors])
    return cors_response(
        req=request,
        status_code=409,
        body=json.dumps(delete_none(error_object.to_dict()), indent=_INDENT, sort_keys=True)
