...
find ${STUB_DIR}/openapi_server \( -type d -name .git -prune \) -o -type f -print0 | xargs -0 sed -i '' 's/openapi_server/archiver.openapi_server/g'
...
while read f; do
    ...
    sed -i '' "s/from openapi_server import util/from archiver.openapi_server import util\\"$'\n'\\"from archiver.openapi_server.response import ${f%???} as rc/g" \
    $WORKING_DIR/controllers/${f}
    while read line; do
        if [[ $line == def* ]]; then
            echo "  - ${line}"
